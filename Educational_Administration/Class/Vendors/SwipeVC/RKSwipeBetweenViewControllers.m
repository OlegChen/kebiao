...
CGFloat X_BUFFER = 80.0; //%%% the number of pixels on either side of the segment
...
CGFloat SELECTOR_Y_BUFFER = 37.0; //%%% the y-value of the bar that shows what page you are on (0 is the top)
...
-(void)setupSegmentButtons {
    ...
    for (int i = 0; i<numControllers; i++) {
        ...
        button.backgroundColor = [UIColor clearColor];//[UIColor colorWithRed:0.03 green:0.07 blue:0.08 alpha:1];//%%% buttoncolors
...
-(void)setupSelector {
    ...
    selectionBar.backgroundColor = RedColor; //%%% sbcolor
            ...
            for (int i = (int)tempIndex+1; i<=button.tag; i++) {
                [pageController setViewControllers:@[[viewControllerArray objectAtIndex:i]] direction:UIPageViewControllerNavigationDirectionForward animated:YES completion:^(BOOL complete){
                    ...
                    if (complete) {
                        ...
                        [weakSelf changeBtnColorWithTag:button.tag];
                    }
                }];
            }
        }
...
- (void)changeBtnColorWithTag:(int)tag{
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView {
    CGFloat xFromCenter = (self.view.frame.size.width-scrollView.contentOffset.x) * (self.view.frame.size.width - 2 * X_BUFFER) / self.view.frame.size.width; //%%% positive for right swipe, negative for left
...
-(void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)finished previousViewControllers:(NSArray *)previousViewControllers transitionCompleted:(BOOL)completed {
    if (completed) {
        ...
        [self changeBtnColorWithTag:self.currentPageIndex];
    }
}
