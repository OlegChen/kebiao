...
- (void)beginLoading{
    ...
    if (!self.loadingView) { //初始化LoadingView
        self.loadingView = [[EaseLoadingView alloc] init];//WithFrame:CGRectMake(0, 0, SCREEN_WIDTH, self.frame.size.height == 0 ? SCREEN_HEIGHT : self.frame.size.height )];
...
#warning --  只考虑了首页的情况
            [self.blankPageView mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.centerX.equalTo(self.blankPageContainer).offset(0);
                make.centerY.equalTo(self.blankPageContainer).offset(0);
            }];
        }else{
...
- (UIView *)blankPageContainer{
    ...
    for (UIView *aView in [self subviews]) {
        if ([aView isKindOfClass:[UITableView class]] || [aView isKindOfClass:[UIScrollView class]]) {
...
//        [_monkeyView setCenter:self.center];
        ...
        _ActivityIndicatorView.cornerRadius = 6;
        _ActivityIndicatorView.backgroundColor = [UIColor colorWithRed:119/255.0 green:119/255.0 blue:119/255.0 alpha:0.7];
...
- (void)configWithType:(EaseBlankPageType)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError reloadButtonBlock:(void (^)(id))block{
    ...
    if (hasError) {
        ...
        if (!_reloadButton) {
            ...
            _reloadButton.borderColor = [UIColor lightGrayColor];
            _reloadButton.cornerRadius = 17.5;
            _reloadButton.borderWidth = 1;
            _reloadButton.titleLabel.font = normalContentFont;
            [_reloadButton setTitleColor:normal_8_ContentColor forState:UIControlStateNormal];
...
#warning 图片不对 ❌
        ...
        switch (blankPageType) {
            ...
            case WelfareBlankPageTypeView: {
                imageName = @"planet";
                tipStr = @"没有优惠券了";
                break;
            }
            case HelperBlankPageTypeView: {
                imageName = @"planet";
                tipStr = @"你还没有上课视频噢~";
                break;
            }
                
            case MyClassBlankPageTypeView: {
                imageName = @"planet";
                tipStr = @"当月没有课程";
                break;
            }
                
            case ClassImgDetailBlankPageTypeView: {
                imageName = @"planet";
                tipStr = @"暂时还没有图文详情";
                break;
            }
            case ClassCommentListBlankPageTypeView: {
                imageName = @"planet";
                tipStr = @"暂时还没有评价";
                break;
            }
            case CartListBlankPageTypeView: {
                imageName = @"planet";
                tipStr = @"购物车中暂未添加商品";
                break;
            }
            case MyCollectionNoClassBlankPageTypeView: {
                imageName = @"planet";
                tipStr = @"暂无数据";
                break;
            }
                
            case MyCollectionNoKnowledgeBlankPageTypeView: {
                imageName = @"planet";
                tipStr = @"暂无数据";
                break;
            }
                
            case MyChildrenBlankPageTypeView: {
                imageName = @"planet";
                tipStr = @"暂时没有学习成员，请添加新成员";
                break;
            }
            case MyOrderBlankPageTypeView: {
                imageName = @"planet";
                tipStr = @"暂无订单";
                break;
            }
            case SearchBlankPageTypeView: {
                imageName = @"planet";
                tipStr = @"未搜索到结果";
                break;
            }
            case AuditonListPageTypeView: {
                imageName = @"planet";
                tipStr = @"暂无试听信息";
                break;
            }
